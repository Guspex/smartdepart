...
- [ ] T041 Run the complete [quickstart.md](./quickstart.md) validation (steps 1–9) against a live IRIS Community Edition container. **Blocked, deeper than previously known**: steps 1-3 (schema, Foreign Table, vector/keyword search mechanics) verified live; step 4 (TRAIN MODEL) blocked by the AutoML crash (T010, reproduced 3x). Step 5 initially looked blocked only by "CLI needs embedded Python" — that part was actually solved (ran `intersystems_pyprod` via `/usr/irissys/bin/irispython` inside the container; `production.py` and all 6 message classes loaded successfully) — but loading any of the 4 host classes (`BS_UberRouteService` etc.) then failed with `ERROR #5351: Class ... does not exist`, root-caused to a genuine IRIS 2026.1 Build 234U bug where new classes with underscores in their name get silently truncated at the first underscore during compilation. Reproduced 3 independent ways (iris_execute, direct CLI, Management Portal import). See research.md §12 for the full diagnostic trail and next-step options. Steps 6-9 not reachable until this is resolved.
