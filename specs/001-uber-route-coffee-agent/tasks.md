...
- [ ] T041 Run the complete [quickstart.md](./quickstart.md) validation (steps 1–9) against a live IRIS Community Edition container. **Progress, then a second, deeper blocker found**: steps 1-3 (schema, Foreign Table, vector/keyword search mechanics) verified live; step 4 (TRAIN MODEL) blocked by the AutoML crash (T010, reproduced 3x). Step 5's "CLI needs embedded Python" issue was solved (ran via `/usr/irissys/bin/irispython` inside the container). The underscore class-name compile bug (research.md §12) was **fixed** by renaming all 4 host classes to underscore-free PascalCase (`BsUberRouteService`, `BpRouteOrchestrator`, `BoIntegratedMlPredictor`, `BoHybridRagEngine`, applied at the user's explicit request) — all 4 classes + `production.py` + all 6 message classes now compile and `Ens.Director.StartProduction` starts the production successfully. However, exercising it (steps 6-8) revealed a **new, deeper issue**: worker jobs are marked "dead" by `Ens.MonitorService` within seconds of receiving their first message — even `BsUberRouteService`, whose handler makes no SQL/external calls, just `send_request_sync(...)`. This is the same class of failure as the T010 AutoML segfault (a job silently dying inside this specific container), pointing to a broader embedded-Python-bridge instability in this container, not a code defect. See research.md §13. Steps 6-9 still not reachable.
